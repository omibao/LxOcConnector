...
在蓝信里和 opencode AI 助手对话——收发消息、随时接管电脑上正在进行的会话。本机主动连出蓝信 WebSocket，**无需公网 IP、域名或内网穿透**。
...
| 📋 **斜杠命令** | `/sessions` `/switch` `/history` `/new` `/exit` `/current` `/help` |
| 🔒 **会话状态感知** | 列表标记 🔒 使用中的会话；接管时自动等待重试 |
| 🔐 **安全鉴权** | fail-closed：未配置允许列表时拒绝所有人 |
...
| `OPENCODE_TIMEOUT` | | `600` | 单次请求超时（秒），大会话需要较长时间 |
| `REQUIRE_MENTION` | | `true` | 群聊是否必须 @机器人 |
| `ALLOW_ALL_USERS` | | `false` | 允许所有用户（调试用） |
| `ALLOWED_USERS` | | — | 允许的用户 openId（逗号分隔），为空时拒绝所有人 |
...
| 命令 | 说明 |
|------|------|
| `/sessions` | 列出所有 opencode 会话（跨项目，按最新对话排序，🔒=使用中，每页 10 条） |
| `/more` | 显示下一页会话 |
| `/switch <序号>` | 接管对应会话，继续对话（使用中自动等待 10 秒重试） |
| `/history [N]` | 查看当前会话最近 N 轮对话（默认 5，一个完整问题+回答为一轮） |
| `/new` | 开始全新会话 |
| `/current` | 查看当前绑定的会话 |
...
0. [my-web-app] 修复登录页 CSS 样式错乱的问题 🔒 使用中)
...
1. 蓝信发 `/sessions` → 看到所有项目的会话列表（🔒 标记正在使用的）
2. 找到要接管的，发 `/switch <序号>`（接管后显示最近一轮对话）
...
LxOcConnector/
├── main.py               # 入口
├── config.py             # .env 配置加载 + fail-closed 鉴权
├── lansenger_inbound.py  # 蓝信 WebSocket 入站监听
├── opencode_client.py    # opencode server HTTP 客户端 + SQLite 会话查询
├── bridge.py             # 桥接逻辑 + 斜杠命令 + 会话管理
...
| opencode 回复慢 | 大会话上下文加载较慢，默认超时 600s；复杂任务需要几分钟 |
| 接管会话后无回复 | 确认会话未被其他客户端占用（🔒 标记）；确认 opencode serve 在运行 |
| 回复内容混入其他客户端输出 | 同一会话不能被两个客户端同时使用，请等一方处理完再接管 |
...
- 同一 opencode 会话不能被两个客户端同时使用（TUI + 蓝信会冲突）

---

## 技术背景

蓝信 WebSocket 协议移植自开源 [`hermes-lansenger-adapter`](https://github.com/lansenger-pm/hermes-lansenger-adapter)（MIT）。出站使用 [`lansenger-sdk`](https://pypi.org/project/lansenger-sdk/)（自动 appToken 刷新）。opencode 后端通过 [Server API](https://opencode.ai/docs/server) 的同步接口 `POST /session/:id/message` 交互。会话列表通过 SQLite 数据库直接查询，支持跨项目列出所有会话。
...
**稳定版发布。** 经过多轮测试修复，核心功能稳定可用。

- **[架构]** 全部改用同步接口（`POST /session/:id/message`），移除 SSE 流式模式，彻底解决跨项目会话卡死、SSE 断开永久等待等问题
- **[功能]** `/sessions` 列表标记 🔒 使用中的会话，一目了然
- **[功能]** `/switch` 遇到 busy 会话自动等待 10 秒重试，不直接拒绝
- **[功能]** `/switch` 接管后显示最近一轮对话（完整问题+回答）
- **[功能]** `/history [N]` 按轮分组查看对话历史（一个完整问题+回答为一轮）
- **[功能]** `/exit` 取消接管当前会话
- **[修复]** 私聊 chat_id 使用完整 openId，修复回复发不出（蓝信返回无权限）
- **[修复]** `_send` 检查 `send_text` 返回值，发送失败不再静默
- **[修复]** `send_prompt` 加 `asyncio.wait_for` 超时保护，会话 busy 时不永久卡住
- **[修复]** `/switch` 时重新查询列表，防止序号偏移导致接错会话
- **[修复]** `/sessions` 会话摘要改用 SQLite 直查，彻底解决"无对话"问题
- **[修复]** 发消息前检查会话 busy 状态，先 abort 再发送
- **[改进]** 超时 600 秒，大会话上下文加载需要时间
- **[测试]** 56 项单元测试全部通过

### v0.6.0

- **[功能]** `/history [N]` 查看当前会话的对话历史（默认 5 条，可指定 1-50 条）
- **[功能]** `/exit` 取消接管当前会话（下次发消息自动新建）
- **[修复]** `/sessions` 会话摘要不再显示"无对话"——长对话中最近几条可能全是 AI 回复，查询 limit 从 6 提到 50
